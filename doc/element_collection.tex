    ...
    \texttt{COLLECTION} can be populated either by a static list of items (\texttt{INSTANCE}, \texttt{ATTRIBUTE}, ..) or by   %joining to another \texttt{COLLECTION} (using the \texttt{JOIN} element).
...
<GLOBALS>
\end{lstlisting}   
%mir : should we add a pointer to one of the git hub examples to be more explicit ? 
...
\begin{table}[!htbp]
 ...
 \end{table}
% mir explain Position  and what are Only , Any 
