...
\begin{lstlisting}[caption={\texttt{WHERE} Example: only rows having val1 as col1 value and  val2 as col2 value must be mapped.},language=XML]
...
\begin{lstlisting}[caption={\texttt{WHERE} Example: the join is satisfied when the value of the '\_pksrcid' column is equal to the '\_srcid' column of the foreign table (see \ref{WHERE_snippet}). },language=XML]
<JOIN dmref="_ts_data">
